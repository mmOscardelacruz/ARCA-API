...
const config = {
  ...
  // cronSafety: {
  //   cronExpressionSafety: process.env.CRON_SAFETY ?? '00 00 * * *'
  // },
  cron: {
  ...
  geotab: {
    ...
    startTime: process.env.fromDate ?? '',
    endTime: process.env.toDate ?? '',
    goAddressURL: process.env.GEO_URL_ADDRESS ?? '',
    goAddressKey: process.env.GEO_KEY_ADDRESS ?? ''
  },
